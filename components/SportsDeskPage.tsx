...
const DATA_SECTIONS: Record<string, DataSection[]> = {
  ...
  nba: [
    { id: "scoreboard", label: "Recent Results & Upcoming Games", kind: "games", sources: ["scores", "schedule"], providerKey: "scores" },
    { id: "standings", label: "Standings", kind: "standings", sources: ["standings"], providerKey: "standings" },
  ],
  fantasy: [],
};
...
const CONTEXT_SECTIONS: Record<string, StorylineSpec[]> = {
  ...
  nba: [
    { id: "offseason", label: "Transactions & Offseason", moduleIds: ["transactions", "contracts", "roster-moves"], required: true },
    { id: "team-player-storylines", label: "Team & Player Storylines", moduleIds: ["injuries", "coaching", "player-development"], required: true },
  ],
};
...
function StoryCard({ story, compact = false }: { story: DeskStory; compact?: boolean }) {
  return (
    <a
      ...
      className={`${compact ? "block" : "block rounded-xl border border-[#dbe4f0] bg-white shadow-sm"} group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315c8d] focus-visible:ring-offset-2`}
    >
      <article className={compact ? "" : "p-5"}>
      ...
      {!compact ? (
        <span className="mt-4 inline-flex text-sm font-bold text-[#315c8d] underline underline-offset-4">
...
function StandingRow({ item }: { item: Record<string, unknown> }) {
  const url = validExternalUrl(item.url) ? item.url : String(item.source_url);
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" aria-label={`${String(item.team)} standings from ${String(item.source)} (opens in a new tab)`} className="grid grid-cols-[1fr_auto_auto] gap-3 border-b border-[#dbe4f0] py-2 text-sm last:border-0 hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315c8d]">
...
function RankingRow({ item }: { item: Record<string, unknown> }) {
  const url = validExternalUrl(item.url) ? item.url : String(item.source_url);
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" aria-label={`${String(item.team)} ranking from ${String(item.source)} (opens in a new tab)`} className="grid grid-cols-[2.5rem_1fr_auto] gap-3 border-b border-[#dbe4f0] py-2 text-sm last:border-0 hover:bg-blue-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#315c8d]">
...
function EmptyState({ label, provider }: { label: string; provider?: string }) {
